...
static constexpr ch ch_list[]{ch::Ch1, ch::Ch2, ch::Ch3, ch::Ch4, ch::Ch5, ch::Ch6, ch::Ch7, ch::Ch8};

template <int32_t _led_begin, int32_t _led_end, typename std::enable_if<(_led_begin > _led_end), bool>::type = true>
static auto pwm_led_blinky(bsd::pwm &, bsd::led &, uint32_t, uint32_t, ...)
{
}

template <int32_t _led_begin, int32_t _led_end, typename std::enable_if<(_led_begin <= _led_end), bool>::type = true>
static auto pwm_led_blinky(bsd::pwm &pwm, bsd::led &led, uint32_t times, uint32_t scale)
{
