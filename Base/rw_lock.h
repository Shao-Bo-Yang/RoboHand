...
struct rw_lock {
  ...
  rw_lock(rw_lock &&other) : _m_lock(nullptr) { _m_lock.swap(other._m_lock); }
 ...
 public:
  ...
  template <rw_lock_operation op>
  auto read_operate(uint32_t wait) {
  ...
  template <rw_lock_operation op>
  auto write_operate(uint32_t wait) {
